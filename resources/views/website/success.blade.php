...
@section('content')
    <section class="section_succes_page">
        <div class="container">
            <div class="msg-confirmed wow fadeInUp">
                <h4>@lang('website.Your Order is confirmed') !</h4>
                <p>@lang('website.An email has been sent to your email address')
                    @if($order->user_id != '')
                        {{@$order->user->email}}
                    @else
                        {{@$order->email}}
                        ...
                        @foreach($order->products as $oneProduct)
                            <div class="item-summary">
                                ...
                                <div class="price-order">
                                    <strong>{{number_format(@$oneProduct->price_offer && ($oneProduct->price_offer > 0) ?? @$oneProduct->price ,3)}}  @lang('website.KWD')</strong>
                                </div>
                            </div>
                        @endforeach
                    </div>
