                ...
                <div class="gutter-b example example-compact">
                    ...
                    <div class="contentTabel">
                        ...
                        <div class="container box-filter-collapse" >
                            <div class="card" >
                                <form class="form-horizontal" method="get" action="{{route('admins.products.index')}}">
                                    <div class="row">
                                        <div class="col-md-4">
                                        ...
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                ...
                                                <select class="select2 form-control" name="category_id" >
                                                    ...
                                                    <option value="{{ $oneCat->id }}" {{ old('category_id', request('category_id')) == $oneCat->id ? 'selected' : ''
                                                        }}>{{@$oneCat->name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <a href="{{ route('admins.products.index') }}" type="submit" class="btn sbold btn-default btnRest">{{__('cp.reset')}}
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover tableWithSearch" id="tableWithSearch">
                                ...
                                <tr>
                                    ...
                                    <th> {{ucwords(__('cp.weight')) .' (' .__('cp.KG') .')'}}</th>
                                    <th> {{ucwords(__('cp.remaining_stock_quantity'))}}</th>
                                    <th> {{ucwords(__('cp.sold_stock_quantity'))}}</th>
                                    <th> {{__('cp.price') . ' ('.__('cp.KWD') .')'}}</th>
                                    <th> {{__('cp.price_offer') . ' ('.__('cp.KWD') .')'}}</th>
                                ...
                                @forelse($items as $one)
                                    <tr class="odd gradeX" id="tr-{{$one->id}}">
                                        ...
                                        <td class="v-align-middle wd-25p">{{$one->weight}}</td>
                                        <td class="v-align-middle wd-25p">{{$one->remaining_quantity}}</td>
                                        <td class="v-align-middle wd-25p">{{$one->quantity - $one->remaining_quantity}}</td>
                                        <td class="v-align-middle wd-25p">{{number_format($one->getRawOriginal('price') , 3)}}</td>
                                        <td class="v-align-middle wd-25p">{{number_format($one->getRawOriginal('price_offer') , 3)}}</td>
