        ...
        <div class="d-flex flex-column-fluid">
            ...
            <div class="container">
                ...
                <div class="card card-custom gutter-b example example-compact">
                    <form method="post" action="{{ route('admins.bannerAdUpdate') }}"
                          enctype="multipart/form-data" class="form-horizontal" role="form" id="form">
                        @csrf

                        <div class="card-header">
                            <h3 class="card-title">{{__('cp.bannerAd')}}</h3>
                        </div>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <select class="select2 form-control" name="type_link" id="typeLink" required>
                                                ...
                                                <option value="1" {{ old('type_link', @$setting->type_link) == 1 ? 'selected' : ''
                                                    }}>{{__('cp.nothing')}}</option>
                                                <option value="2" {{ old('type_link', @$setting->type_link) == 2 ? 'selected' : ''
                                                    }}>{{__('cp.product')}}</option>
                                                <option value="3" {{ old('type_link', @$setting->type_link) == 3 ? 'selected' : ''
                                                    }}>{{__('cp.main_category')}}</option>
                                                <option value="4" {{ old('type_link', @$setting->type_link) == 4 ? 'selected' : ''
                                    ...
                                    <div class="col-md-6" id="products">
                                        <div class="form-group">
                                            ...
                                            <select class="select2 form-control" name="product_id" id="product_id">
                                                ...
                                                <option value="{{ $product->id }}" {{ (intval(old('product_id', @$setting->linked_id)) ===
                                                    intval($product->id) &&
                                                    intval(old('type_link', @$setting->type_link)) === 2) ? 'selected' : '' }}>
                                                    {{ $product->name }}
                                                </option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="col-md-6" id="categories">
                                        <div class="form-group">
                                            ...
                                            <select class="select2 form-control" name="category_id" id="category_id">
                                                ...
                                                <option value="{{ $oneCat->id }}" {{ (intval(old('category_id', @$setting->linked_id)) ===
                                                    intval($oneCat->id) && intval(old('type_link', @$setting->type_link)) === 3) ? 'selected' : '' }}>
                                                    {{ $oneCat->name }}
                                                </option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="col-md-6" id="subCategories">
                                        <div class="form-group">
                                            ...
                                            <select class="select2 form-control" name="sub_category_id" id="sub_category_id">
                                                ...
                                                <option value="{{ $oneSub->id }}" {{ (old('linked_id', @$setting->linked_id) == $oneSub->id &&
                                                    @$setting->type_link == 4) ? 'selected' : '' }}>
                                                    {{ $oneSub->name }}
                                                </option>
                                                @endforeach
                                        ...
                                        <div class="row">
                                            <div class="col-md-12" >
                                                <div class="fileinputForm">
                                                    ...
                                                    <label>{{__('cp.image')}} <span style="display: inline-block; color: #2c3e50; background: linear-gradient(to right, #ecf0f1, #bdc3c7); padding: 3px 8px; border-radius: 4px; font-size: 0.9em; box-shadow: 1px 1px 3px rgba(0,0,0,0.1);">( 1628px X 500px)</span></label>

                                                    <div class="fileinput-new thumbnail" onclick="document.getElementById('edit_image').click()"
                                                        style="cursor:pointer">
                                                        <img src="{{@$setting->banner_ad_image ? @$setting->banner_ad_image :  admin_assets('images/ChoosePhoto.png')}}"
                                                            id="editImage">
                                                    </div>
                                                    <div class="btn btn-change-img red" onclick="document.getElementById('edit_image').click()">
                                                        <i class="fas fa-pencil-alt"></i>
                                                    </div>
                                                    <input type="file" class="form-control" name="banner_ad_image" id="edit_image" style="display:none">
                                                </div>
                                            </div>
...
<script>
    $(document).ready(function () {
        ...
        $("#typeLink").val("{{ old('type_link', @$setting->type_link) }}").trigger("change");
    });
</script>
@endsection
