...
class CategoryService
{
    ...
    public function updateCategory(Category $item, $data): void
    {
        DB::transaction(function () use ($data, $item) {
            ...
            $item->is_featured = isset($data->is_featured) && $item->subcategories->isEmpty() && $data->is_featured == "on" ? 'yes' : 'no';
    ...
    public function createSubCategory($data): void
    {
        DB::transaction(function () use ($data) {
            ...
            $this->updateParentCategoryFeaturedStatus($data->parent_id);
            storeTranslatedFields($newItem, ['name', 'key_words'], $data);
            $newItem->save();
        });
    }
    ...
    private function updateParentCategoryFeaturedStatus($parentId): void
    {
        if ($parentId) {
